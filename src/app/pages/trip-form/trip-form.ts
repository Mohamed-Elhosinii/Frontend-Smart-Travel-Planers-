...
export class TripForm {
  ...
  decrease(field: string) {
    const value = this.form.get(field)?.value;
    const minValues: any = { adults: 1, children: 0, rooms: 1 };
  ...
  toggleStyle(style: string) {
    ...
    if (current.includes(style)) {
      this.form.patchValue({ travelStyle: current.filter((s: string) => s !== style) });
    } else {
      this.form.patchValue({ travelStyle: [...current, style] });
    }
  }
